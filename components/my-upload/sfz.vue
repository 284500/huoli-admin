<template>
  <input type="file" class=" absolute top-0 left-0 right-0 bottom-0 opacity-0 z-10" />
  <img v-if="modelValue" class="absolute w-full h-full left-0 top-0" :src="value">
  <img v-else :src="props.iconUrl" alt="" class="w-full h-full absolute top-0">
  <div v-else class="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] flex flex-col gap-2 items-center">
...
const props = defineProps({
  iconUrl:{
    type: String,
    default:'/public/img/auth/sfz-2.png'
  },
  title:{
    type: String,
    default: '请上传身份证正面'
