<template>
    ...
    <div class="bg-white rounded-[8px] md:p-10 p-4 header-bg" v-if="!material.status">
      <div class="flex sm:justify-between flex-wrap justify-end gap-4">
        <div class="flex gap-4">
          <div class="w-12 h-12"><img src="/public/img/auth/status-true.png" class="w-full h-full object-cover" /></div>
          <div>
            <h1 class="title">订单提交成功，请尽快付款！</h1>
            <div><span class="text">请您在</span><span class="text !text-[#FF5030]" >2天内</span><span class="text">支付完成，否则订单会被自动取消。</span>
            </div>
            <div><span class="text">剩余时间：</span>
              <span class="text !text-[#FF5030]" >{{countDownTime.days}}</span><span class="text">天</span>
              <span class="text !text-[#FF5030]" >{{countDownTime.hours}}</span><span class="text">小时</span>
              <span class="text !text-[#FF5030]" >{{countDownTime.minutes}}</span><span class="text">分钟</span>
              <span class="text !text-[#FF5030]" >{{countDownTime.seconds}}</span><span class="text">秒</span>
      ...
      <div class="grid gap-4 grid-cols-2 mt-4">
        <div class="table-border sm:p-4 p-2"
  ...
  <Drawer v-model="isShow">
    <div
      class="bg-white w-[560px] md:h-[608px] max-w-[100vw] max-h-[100vh] rounded-[8px] h-[500px] relative  px-6 pb-3 pt-3.5">
      <div class="w-full h-full flex flex-col justify-between items-center">
        <div class="flex justify-between w-full">
          <div class="table-title">扫码支付</div>
          <div><img src="/public/img/login/close.png" class="w-6 h-6 " @click="isShow = false" /></div>
        </div>
...
const countDownTime=computed(()=>{
  let time=material.value.createdTime*1000+24*2*60*60*1000-timestamp.value;
