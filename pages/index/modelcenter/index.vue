<template>
  <Myheader>
    <template #left>
      <MyInputSearch
        v-model="Materialparams.name"
        @search="Material.resetPage()"
  ...
  <ScrollArea class="flex-1 overflow-auto">
    <div
      class="w-[100vw] px-4 relative md:px-8 lg:px-12 box-border lg:w-full min-h-[calc(100vh-76px)] flex flex-col justify-between"
    >
      <div class="w-full">
        ...
        <div v-if="templateactive === 0">
          ...
          >
            <MyList
              :lists="Product.types.data"
              title="类型"
              v-model="Materialparams.productType"
              @change="Material.resetPage()"
            ></MyList>
            <MyList
              :lists="Industry.types.data"
              title="行业"
              v-model="Materialparams.industryTypeList"
              @change="Material.resetPage()"
            ></MyList>
          </div>
          ...
          <div v-if="Material.pager.loading" class="flex flex-col items-center py-20">
            <!-- <div>加载中</div> -->
            <div>
              <Lucide icon="Loader2" class="w-10 h-10 mt-2 animate-spin" color="blue"></Lucide>
            </div>
          </div>
              ...
              <div v-for="(item, index) in Material.pager.lists" :key="index">
                <div class="relative card bg-muted pt-[66%]">
                  <div class="absolute top-0 left-0 bottom-0 right-0 object-cover">
                    <img :src="item.templateCover" alt="" class="w-full h-full object-cover" />
                  </div>
        ...
        <div v-if="templateactive === 1">
          ...
          >
            <MyList
              :lists="Product.types.data"
              title="类型"
              v-model="H5params.productType"
              @change="H5.resetPage()"
            ></MyList>
            <MyList
              :lists="Industry.types.data"
              title="行业"
              v-model="H5params.industryTypeList"
              @change="H5.resetPage()"
            ></MyList>
            <MyList
              :lists="Purposes.types.data"
              title="用途"
              v-model="H5params.applicablePurposeList"
              @change="H5.resetPage()"
            ></MyList>
          </div>
          <div class="flex justify-between py-3 items-center">
...
import { getH5TemplateDetail, getH5TemplateList } from '@/server/apis/template/h5.js';
import {
